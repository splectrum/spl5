...
let types = require('../avsc/lib/types'),
    utils = require('../avsc/lib/utils'),
    avscPlatform = require('../avsc/lib/platform'),
    compat = require('./compat'),
    buffer = require('bare-buffer'),
    events = require('bare-events'),
    stream = require('bare-stream');

// Ensure process global (nextTick) for Bare
compat.ensureProcess();

// Bridge: v6 platform + v5 compat shims
let platform = Object.assign({}, avscPlatform, compat);

// Wrap getHash in utils to return Buffer (v6 returns Uint8Array)
utils.getHash = compat.wrapGetHash(avscPlatform.getHash);

// Restore removed utils functions for v5 services compatibility
if (!utils.addDeprecatedGetters) utils.addDeprecatedGetters = compat.addDeprecatedGetters;
if (!utils.newBuffer) utils.newBuffer = compat.newBuffer;
if (!utils.bufferFrom) utils.bufferFrom = compat.bufferFrom;
...
let Tap = utils.Tap;
let Type = types.Type;
let debug = platform.debuglog('avsc:services');

// Various useful types. We instantiate options once, to share the registry.
let OPTS = {namespace: 'org.apache.avro.ipc', registry: {}};
...
Message.prototype.isOneWay = platform.deprecate(
...
class Service {
  ...
  static forProtocol (ptcl, opts) {
    ...
    // Shared registry for all types in this protocol (v6 compat:
    // Type.forSchema copies opts, but shares the registry object)
...
Service.prototype.createEmitter = platform.deprecate(
...
Service.prototype.createListener = platform.deprecate(
  function (transport, opts) {
...
Service.prototype.emit = platform.deprecate(
...
Service.prototype.equals = platform.deprecate(
  function (any) {
    return (
...
Service.prototype.getFingerprint = platform.deprecate(
  function (algorithm) {
...
Service.prototype.getSchema = platform.deprecate(
  Type.prototype.getSchema,
  'use `.protocol` instead of `.getSchema()`'
);

Service.prototype.on = platform.deprecate(
...
Service.prototype.subprotocol = platform.deprecate(
  function () {
...
ClientChannel.prototype.getCache = platform.deprecate(
  function () { return this.client._cache$; },
  'use `.remoteProtocols()` instead of `.getCache()`'
);

ClientChannel.prototype.getProtocol = platform.deprecate(
  function () {
...
ClientChannel.prototype.isDestroyed = platform.deprecate(
...
class ServerChannel extends events.EventEmitter {
  ...
  _createHandshakeResponse (err, hreq) {
    ...
    let buf = svc.hash;
    let serverMatch = hreq && utils.bufEqual(hreq.serverHash, buf);
    return {
...
ServerChannel.prototype.getCache = platform.deprecate(
  function () { return this.server._cache; },
  'use `.remoteProtocols()` instead of `.getCache()`'
);

ServerChannel.prototype.getProtocol = platform.deprecate(
  function () {
...
ServerChannel.prototype.isDestroyed = platform.deprecate(
  function () { return this.destroyed; },
  'use `.destroyed` instead of `.isDestroyed`'
);
...
function readHead(type, buf) {
  ...
  return {head, tail: (tap.arr || tap.buf).slice(tap.pos)};
}
